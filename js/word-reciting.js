...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function loadNextWord() {
        console.log('加载下一个单词，当前索引:', currentState.currentWordIndex, '总单词数:', wordList.length);
        
        if (currentState.currentWordIndex >= wordList.length) {
            console.log('所有单词学习完成，显示完成页面');
            showCompletion();
            return;
        }
        
        const word = wordList[currentState.currentWordIndex];
    ...
    function handleFeedback(isKnown) {
        ...
        const currentWord = wordList[currentState.currentWordIndex];
        ...
        // 更新任务进度（如果是任务模式）
        if (urlParams.task) {
            const progress = Math.round((currentState.currentWordIndex / wordList.length) * 100);
    ...
    function updateProgress() {
        ...
        // 计算进度百分比
        const progressPercent = (currentState.currentWordIndex / wordList.length) * 100;
        ...
        // 更新进度文本
        if (progress) {
            progress.textContent = `进度: ${currentState.currentWordIndex}/${wordList.length}`;
    ...
    function reviewWrongWords() {
        ...
        // 用错题列表替换单词列表
        wordList.length = 0;
        wordList.push(...currentState.wrongWords);
    ...
    function loadTaskWords(taskId) {
        ...
        if (task.words && task.words.length > 0) {
            wordList.length = 0;
            wordList.push(...task.words.map(word => ({
                word: word.word,
                phonetic: word.phonetic,
                meanings: word.meanings,
                difficulty: word.difficulty
            })));
            console.log('加载了', wordList.length, '个任务单词');
        } else {
        ...
        // 更新进度显示
        if (progress) {
            progress.textContent = `进度: 0/${wordList.length}`;
        }
